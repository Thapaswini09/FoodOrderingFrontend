...
const Cart = () => {
  ...
  const getCartsData = async () => {
    const res = await axios.get(`${API_BASE}/api/user-get-carts`, {
  ...
  const handleCheckout = async () => {
    try {
      const cartIds = itemsdata.flatMap((order) =>
        order.carts.map((cart) => cart._id)
      );
      if (cartIds.length === 0) return alert("No items to checkout.");

      const res = await axios.post(
        `${API_BASE}/api/all-order-data`,
        { cartIds },
        {
          withCredentials: true,
          headers: {
  ...
  const handleSingleCheckout = async (cartId) => {
    try {
      const res = await axios.post(
        `${API_BASE}/api/single-order-data`,
        { id: cartId },
        {
          withCredentials: true,
          headers: {
  ...
  const handleRemoveItem = async (id) => {
    const res = await axios.delete(`${API_BASE}/api/user-delete-carts/${id}`, {
      withCredentials: true,
      headers: {
        "Content-Type": "application/json",
        "x-access-token": Cookies.get("userToken"),
    ...
    <div className="container my-4">
      ...
      <div className="table-responsive">
        <table className="table table-bordered table-hover text-center align-middle bg-white">
          ...
          <tbody>
            {itemsdata.length === 0 ||
            itemsdata.every((order) => order.carts.length === 0) ? (
              <tr>
                <td
                  colSpan="7"
