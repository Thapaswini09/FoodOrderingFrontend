...
const UserOrders = () => {
  ...
  const getUserOrderDeatils = async () => {
    try {
      ...
      const res = await axios.get(`${API_BASE}/api/user-get-orders`, {
    ...
    <>
      ...
      >
        ...
        <tbody className="orders-table-body">
          {ordersData.length === 0 ||
          ordersData.every((item) => item.orders.length === 0) ? (
            <tr>
              ...
              return ordersData.flatMap((item, index) =>
                item.orders.map((order, orderIndex) => {
                  ...
                  return (
                    ...
                    >
                      <td className="orders-table-data order-index">
                        {orderCounter++}
                      </td>
                      <td className="orders-table-data resturent-name">
                        {food.resturentName}
                      </td>
                      <td className="orders-table-data food-name">
                        {food.foodName}
                      </td>
                      ...
                      <td className="orders-table-data quantity">
                        {order.quantity}
                      </td>
                      <td className="orders-table-data price">
                        ₹{food.foodPrice}
                      </td>
                      <td className="orders-table-data total-price">
                        ₹{totalPrice}
                      </td>
