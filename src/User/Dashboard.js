...
//                 const imgSrc = ele.foodImage.startsWith("/uploads")
//                   ? `${API_BASE}${ele.foodImage}` // backend uploaded
//                   : `/images/${ele.foodImage}`; // frontend image
...
const Dashboard = () => {
  ...
  const getImageUrl = (image) => {
    if (!image) return "/images/default.png"; // fallback image
    if (image.startsWith("http")) return image; // full URL from backend
    if (image.includes("uploads")) return `${API_BASE}/${image}`; // backend uploads
    return `/images/${image}`; // frontend public folder
  };
