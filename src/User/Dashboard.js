...
const Dashboard = () => {
  ...
  const getAllItems = async () => {
    try {
      ...
      // Prepend API_BASE to images for deployment
      const updatedData = res.data.data.map((item) => ({
        ...item,
        foodImage: `${API_BASE}${item.foodImage}`,
      }));
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <input
          ...
          placeholder="Search items..."
          onChange={(e) => setSearch(e.target.value)}
          style={{ padding: "8px", marginLeft: "20px", width: "250px" }}
        />
      </div>

      <div className="container">
        {itemsdata.length === 0 ? (
          <div style={{ padding: "100px 0", fontSize: "24px", color: "black" }}>
            No data found
          </div>
        ) : (
          <div className="row">
            {itemsdata.map((ele) => (
              <div key={ele._id} className="col-lg-4 col-md-6 col-sm-12 mb-4">
                <div
                ...
                >
                  ...
                  <div className="card-body d-flex flex-column">
                    <h5>
                      Food Name: <b>{ele.foodName}</b>
                    </h5>
                    <p>Price: ₹{ele.foodPrice}</p>
                    <p>Restaurant: {ele.resturentName}</p>
                    <p>
                      {ele.foodDescription.length > 100
                        ? ele.foodDescription.substring(0, 100) + "..."
                        : ele.foodDescription}
                    ...
                    <div className="mt-auto">
                      <div className="d-flex justify-content-center align-items-center gap-4 mb-2">
                        <button onClick={() => handleDecrease(ele._id)}>
                          -
                        </button>
                        <span>{quantities[ele._id] || 1}</span>
                        <button onClick={() => handleIncrease(ele._id)}>
                          +
                        </button>
                      </div>
